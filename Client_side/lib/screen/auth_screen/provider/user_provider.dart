...
class UserProvider extends ChangeNotifier {
  ...
  Future<String?> register() async {
    ...
    try {
      ...
      if (response.isOk) {
        // Parse response - handle both Map and direct access
        ...
        // Check success directly from response body
        if (responseBody['success'] == true) {
          log('register success');
          return null; // Return null to indicate success
        } else {
          return 'Failed to register: ${responseBody['message'] ?? 'Unknown error'}';
        }
      } else {
        // Handle error response
        final errorBody = response.body as Map<String, dynamic>?;
        return 'Error: ${errorBody?['message'] ?? response.statusText ?? 'Registration failed'}';
      }
    } catch (e) {
      log(e.toString());
      return 'An error occurred: $e';
